...
\end{equation}
\begin{equation}
	ST_{ed}- \mathrm{ST}\sum_{b=1}^{\mathrm{NB}}A_{bed} = 0, \quad \forall{e, d}
	\label{eq:setupTimeCalc}
\end{equation}
\begin{equation}
	\sum_{b=1}^{\mathrm{NB}}\{A_{bed}*T_{bd}\} + ST_{ed} - \mathrm{C}_e \leq 0 \quad \forall{e, d}
	\label{eq:timeRest}
\end{equation}
\begin{equation}
...
\footnotesize
	\begin{tabular}{ l l } 
		...
		$c, k$ & used to designate each specifc colour, $c, k = 1, ..., \mathrm{NC}$ \\
